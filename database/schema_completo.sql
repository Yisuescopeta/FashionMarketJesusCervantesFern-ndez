...
-- ============================================
-- 5.1 TABLA DE VARIANTES (STOCK POR TALLA)
-- ============================================
...
create table orders (
    ...
    -- Campos financieros y de sesión
    total_amount integer not null check (total_amount > 0), -- Reemplaza a 'total'
    stripe_session_id text,
    customer_email text,
    
    -- Datos de envío
    shipping_address text default 'No especificada',
    shipping_city text default 'No especificada',
    shipping_postal_code text default '00000',
    shipping_phone text,
    notes text,
...
-- ============================================
-- 11.3.1 POLÍTICAS PARA VARIANTES DE PRODUCTO
-- ============================================
-- Cualquiera puede ver variantes (stock por talla)
...
-- ============================================
-- 11.7 FUNCIONES DE CONTROL DE STOCK POR TALLA
-- ============================================

-- Función para reducir stock por talla específica
...
-- Función para obtener stock de todas las tallas de un producto
...
begin
    ...
    order by 
        case pv.size
            ...
            when '34' then 10 when '36' then 11 when '38' then 12
            else 99
        end;
end;
$$ language plpgsql security definer;

-- Trigger para sincronizar stock total del producto
...
-- ============================================
-- 13.2 STOCK POR TALLA (VARIANTES)
-- ============================================
-- Insertar stock por talla para cada producto
-- El stock se distribuye de forma realista entre las tallas
...
declare
    ...
    idx integer;
begin
    for prod in select id, sizes from products where sizes is not null and array_length(sizes, 1) > 0
    loop
        idx := 1;
        foreach size_item in array prod.sizes
        loop
            -- Distribuir stock de forma variada (algunas tallas con poco stock)
            insert into product_variants (product_id, size, stock)
            values (
                prod.id, 
                size_item,
                -- Stock variado: tallas centrales más stock, extremas menos
                case 
                    when size_item in ('M', 'L', '32', '34') then 25 + (random() * 15)::int
                    when size_item in ('S', 'XL', '30', '36') then 15 + (random() * 10)::int
                    when size_item in ('XS', 'XXL', '28', '38') then 5 + (random() * 10)::int
