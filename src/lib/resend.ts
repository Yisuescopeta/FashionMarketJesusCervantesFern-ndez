...
// Try multiple ways to load the Resend API key (Astro env handling varies by context)
...
if (!resendApiKey) {
  console.error('❌ CRITICAL: RESEND_API_KEY not found. Emails will fail to send.');
}

// Cliente de Resend para envío de emails transaccionales (usa fallback dummy si no existe key para que no de crash en build/dev)
export const resend = new Resend(resendApiKey || 're_dummy_key_123456789');

// Configuración del remitente
export const EMAIL_FROM = 'Aurum <onboarding@resend.dev>';
...
// Función para enviar email de informe administrativo
export async function sendAdminReportEmail(data: {
  adminEmail: string;
  subject: string;
  html: string;
}) {
  try {
    // Si no hay API key real de Resend configurada, usamos Nodemailer (SMTP) como fallback
    if (!resendApiKey || resendApiKey.startsWith('re_dummy_key')) {
      console.log('Utilizando Nodemailer como fallback para enviar informe...');
      const info = await transporter.sendMail({
        from: `"Aurum Reportes" <${process.env.EMAIL_USER || import.meta.env.EMAIL_USER || 'admin@aurum.com'}>`,
        to: data.adminEmail,
    ...
    if (error) {
      console.error('Error enviando informe admin:', error);
      return { success: false, error };
    }
