        ...
        const loadStock = async () => {
            ...
            if (variants) {
                ...
                variants.forEach(variant => {
                    cache[`${variant.product_id}-${variant.size}`] = variant.stock;
                });
                setStockCache(cache);
            }
        };
                        ...
                        $cartItems.map((item) => (
                            <div key={item.id} className="flex space-x-4">
                                <div className="w-20 h-24 bg-slate-100 flex-shrink-0">
                                    <img src={getCloudinaryImageUrl(item.image || '', { width: 160, height: 192, crop: 'fill' })} alt={item.name} className="w-full h-full object-cover" />
                                </div>
